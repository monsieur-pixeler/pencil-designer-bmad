...
Ask using the prompt with its default value. Apply `result` templates when storing (e.g. the result template from module.yaml). Fields with `user_setting: true` go exclusively to `config.user.yaml`.
